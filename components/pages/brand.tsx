...
const Brand = () => {
    ...
    const getFileNameForPath = (path: string) => path.slice(path.lastIndexOf("/") + 1)
    ...
    const downloadAll = () => {
        ...
        let promises = Object.entries(ROOCH_ASSETS).map((k) => k[1].map((path) => {
    ...
    return (
        <>
            ...
            >
                {
                    isDownload ? en ? "Downling" : "下载中" : en ? "DOWNLOAD ASSETS (.ZIP FILE)" : "下载资源 (.ZIP 文件)"
                }
            </button>
            ...
            {Object.entries(ROOCH_ASSETS).map((v) => (
                ...
                <div key={v[0]} className=" border-b pt-10">
                    <h3 className="text-2xl ">{getFileNameForPath(v[0]).toLocaleUpperCase()}</h3>
