...
2. When a user migrates state from Rooch to Layer1, the state is first locked and waits for the Layer2 are finalized (Challenge period to pass or zkProver provide valid proof) before being moved out of the state tree for settlement.
...
We depend on the solution of sync Ethereum state root to other chains. This is a challenge that all cross-chain solutions face. There are some solutions to this problem, such as Ethereum light nodes, Oracles, etc. We can explore this together.
